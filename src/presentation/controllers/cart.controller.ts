...
export class CartController {
  ...
  addItem = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    reply.status(201).send(await this.service.addItem(
      request.user.sub, parsed.data.product_id, parsed.data.quantity
    ));
  };
  ...
  setItem = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!parsed.success) throw new ValidationError('Invalid input', parsed.error.flatten());
    reply.send(await this.service.setItem(
      request.user.sub, Number(productId), parsed.data.quantity
    ));
  };
